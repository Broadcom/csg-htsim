...
STRAT=ecmp # hash-based ECMP packet spraying
PATHS=256 # 256 entropy values
QUEUESIZE=1000 # large enough queue we won't drop packets
...
CMD="$SIMPATH/htsim_ndp -tm $TMFILE -log sink -linkspeed ${LINKSPEED} -strat $STRAT -paths $PATHS -nodes $N -conns $FLOWS -q $QUEUESIZE -cwnd $CWND -mtu $MTU -end ${XMAX} -logtime 0.01 > out.tmp"
echo $CMD
eval $CMD
